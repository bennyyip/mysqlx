defmodule QueryTest do
  ...
  test "queries are dequeued after previous query is processed", context do
    ...
    capture_log(fn ->
      assert %Mysqlx.Error{} = query("DO SLEEP(0.1)", [], timeout: 0)
  ...
  #   :ok = query(sql, [])
  #   insert = ~s{INSERT INTO #{table} (active, title, body, data) } <> ~s{VALUES (?, ?, ?, ?)}
  ...
  #   insert =
  #     ~s{INSERT INTO #{table} (count, intensity, accuracy, value) } <> ~s{VALUES (?, ?, ?, ?)}
  ...
  #   assert query("SELECT count from #{table} WHERE id = LAST_INSERT_ID()", []) == [
  #            [negative_integer]
  #          ]
  ...
  #   [[query_negative_float]] =
  #     query("SELECT intensity from #{table} WHERE id = LAST_INSERT_ID()", [])
  ...
  #   :ok = query(sql, [])
  #   insert = ~s{INSERT INTO #{table} (count, accuracy, value) } <> ~s{VALUES (?, ?, ?)}
  ...
  #   assert query("SELECT count, accuracy, value from #{table} WHERE id = ?", [2]) == [
  #            [nil, double, nil]
  #          ]
  # end
  ...
  #   insert =
  #     ~s{INSERT INTO #{table} (count_1, count_2, count_3, count_4, count_5, } <>
  ...
  #   assert query("SELECT cost FROM #{table} WHERE id = LAST_INSERT_ID()", []) == [
  #            [Decimal.new("-164.9000")]
  #          ]

  #   :ok = query("INSERT INTO #{table} (cost) values (?)", [Decimal.new("16400")])

  #   assert query("SELECT cost FROM #{table} WHERE id = LAST_INSERT_ID()", []) == [
  #            [Decimal.new("16400.0000")]
  #          ]
  # end
  ...
  #   assert query("SELECT t1, t2 FROM #{table} WHERE id = ?", [1]) == [
  #            [~T[19:27:30], ~T[10:14:16]]
  #          ]
  ...
  #   # Datetime
  #   # Only MySQL 5.7 supports microseconds storage, so it will return 0 here
  #   assert query("SELECT dt1, dt2 FROM #{table} WHERE id = 1", []) == [
  #            [~N[2010-10-17 10:10:30], ~N[2010-10-17 13:32:15]]
  #          ]
  ...
  #   # Timestamp
  #   # Only MySQL 5.7 supports microseconds storage, so it will return 0 here
  #   assert query("SELECT ts1, ts2 FROM #{table} WHERE id = 1", []) == [
  #            [~N[2010-10-17 10:10:30], ~N[2010-10-17 13:32:15]]
  #          ]
  ...
  #   assert query("SELECT timestamp('2013-12-21 23:01:27')", []) == [
  #            [{{2013, 12, 21}, {23, 1, 27, 0}}]
  #          ]

  #   assert query("SELECT timestamp('2013-12-21 23:01:27 EST')", []) == [
  #            [{{2013, 12, 21}, {23, 1, 27, 0}}]
  #          ]
  # end
  ...
  #   max_signed = 8_388_607
  #   min_signed = -8_388_608
  #   # out_of_range = max_signed + 1
  #   :ok = query("INSERT INTO #{table} (id, testfield) values (1, ?)", [max_signed])
  #   :ok = query("INSERT INTO #{table} (id, testfield) values (2, ?)", [min_signed])
  #   # Do not work since MySQL 5.7.9, bit test is rather invalid
  ...
  #   min_signed = -9_223_372_036_854_775_808
  #   # out_of_range = max_signed + 1
  #   :ok = query("INSERT INTO #{table} (id, testfield) values (1, ?)", [max_signed])
  #   :ok = query("INSERT INTO #{table} (id, testfield) values (2, ?)", [min_signed])
  #   # Do not work since MySQL 5.7.9, bit test is rather invalid
  ...
  # test "columns list uses alias as table name when inclue_table_name option is specified",
  #      context do
  ...
  #   assert query("SELECT id, text FROM test_statements WHERE id > ?", [0]) == [
  #            [1, "test1"],
  #            [2, "test2"]
  #          ]
  # end
  ...
  #   assert %ArgumentError{message: "parameters must be of length 0 for query" <> _} =
  #            catch_error(query("SELECT 1", [:badparam]))

  #   assert %ArgumentError{message: "query has invalid parameter :badparam"} =
  #            catch_error(query("SELECT ?", [:badparam]))
  # end
  ...
  #   :ok =
  #     query(
  #       "CREATE TABLE test_nullbit (id int, t1 text, t2 text, t3 text, t4 text, t5 text not NULL, t6 text, t7 text not NULL)",
  #       []
  #     )

  #   :ok =
  #     query("INSERT INTO test_nullbit VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [
  #       nil,
  #       "t1",
  #       nil,
  #       "t3",
  ...
  #   assert query("SELECT * FROM test_nullbit WHERE t1 = 't1'", []) == [
  #            [nil, "t1", nil, "t3", nil, "t5", nil, "t7"]
  #          ]
  # end
  ...
  #   sql =
  #     "CREATE TABLE test_replace (id INT UNSIGNED NOT NULL AUTO_INCREMENT," <>
